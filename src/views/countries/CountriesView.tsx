...
const RINGFENCED_ROW_CLASS = 'bg-emerald-50 hover:bg-emerald-100';
...
export function CountryView({ country }: CountryViewProps) {
  ...
  const columns = useMemo((): ColumnDef<Employee, unknown>[] => [
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          {row.original.ringfenced && (
            <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700 cursor-default select-none">
              ISG
            </span>
          )}
        </div>
    ...
    },
    {
      ...
      header: 'Escenario',
      cell: ({ row }) => {
        const { scenarioType, ringfenced } = row.original;
        if (ringfenced) {
          return <Badge variant="green">ISG Ringfenced</Badge>;
        }
        return (
          <Badge variant={scenarioType === 'effective' ? 'blue' : 'neutral'}>
            {scenarioType === 'effective' ? 'Efectivo' : 'Estimación'}
          </Badge>
        );
      },
    },
  ...
  function handleExport() {
    const rows = filtered.map((e) => ({
      ...
      'ISG Ringfenced': e.ringfenced ? 'Sí' : 'No',
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
        ...
        getRowClassName={(e) =>
          `${e.ringfenced ? RINGFENCED_ROW_CLASS : (rowStatusClass[e.chargeabilityStatus] ?? 'hover:bg-[var(--G6)]')}${e.isOnPTO ? ' opacity-50' : ''}`
        }
        pagination={{
          page: safePage,
          pageSize,
                  ...
                  <AnimatePresence mode="sync">
                    {(scenario ? paged.filter((e) => e.scenarioType === scenario) : paged).map((e, index) => (
                      <motion.tr
                        ...
                        animate="show"
                        exit="exit"
                        className={`group border-b border-[var(--G6)] ${e.ringfenced ? 'bg-emerald-50 hover:bg-emerald-100' : 'hover:bg-[var(--G6)]'}`}
                      >
                        <td className="py-2 pr-4 sticky left-0 bg-white">
                          <div className="flex items-center gap-1.5">
                            ...
                            {e.ringfenced && (
                              <span className="text-[9px] font-semibold px-1 py-0.5 rounded bg-emerald-100 text-emerald-700">
                                ISG
                              </span>
                            )}
                            {e.scenarioType === 'assumption' && !e.ringfenced && (
                              <button
